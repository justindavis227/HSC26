-- Secret Page tickets: sequential numbering, tiers, per-device dedup.
--
-- Each correct submission claims the next ticket number (atomic, no gaps/dupes):
--   1            -> gold   (winner; only ticket with the GroupMe CTA)
--   2..50        -> silver
--   51..100      -> bronze
--   101..3500    -> blue
-- At 3500 issuing closes (status 'sold_out'). A device that already claimed a
-- ticket gets the SAME ticket back (one ticket per device, keyed on a local id).
--
-- The correct code stays server-side (camp_info `secret_page_password_2`).
-- Matching is case-insensitive; spaces are significant.

ALTER TABLE public.secret_submissions
  ADD COLUMN IF NOT EXISTS ticket_number int,
  ADD COLUMN IF NOT EXISTS tier text,
  ADD COLUMN IF NOT EXISTS device_id text;

-- Replace the old text-returning function with a jsonb ticket claimer.
DROP FUNCTION IF EXISTS public.submit_secret_response(text, text, text, text);
...
BEGIN
  ...
  -- Serialize ticket assignment.
  PERFORM pg_advisory_xact_lock(hashtext('hsc26_secret_winner'));

  -- One ticket per device: if this device already claimed, return that ticket.
